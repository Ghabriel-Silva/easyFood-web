...
import { useMutateSetStatusProducts } from "../../hooks/useMutateSetStatusProducts"
...
export const UpdateStatus = ({ id, statusDefault }: PropsSelectStatus) => {
    const converteValue = statusDefault ? 'active' : 'inactive'
    const [value, setValue] = useState<string[]>([converteValue])

    const {mutate} = useMutateSetStatusProducts()
    return (
        <Select.Root
            size={"xs"}
            maxWidth={"130px"}
